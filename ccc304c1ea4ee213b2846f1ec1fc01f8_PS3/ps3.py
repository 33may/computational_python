...
from ccc304c1ea4ee213b2846f1ec1fc01f8_PS3.ps3_verify_movement27 import test_robot_movement
...
class Position(object):
    ...
    def get_x(self, round = False):
        return math.floor(self.x) if round else self.x

    def get_y(self, round = False):
        return math.floor(self.y) if round else self.y
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        """
        self.floor = np.full((height, width), dirt_amount)
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        self.floor[pos.get_y(True), pos.get_x(True)] = max(self.floor[pos.get_y(True), pos.get_x(True)] - capacity, 0)
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.floor[n, m] == 0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return np.sum(self.floor == 0)
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        return self.floor.shape[0] * self.floor.shape[1]
    ...
    def get_random_position(self):
        ...
        """
        pox = random.randint(0, self.floor.shape[1] - 1)
        poy = random.randint(0, self.floor.shape[0] - 1)
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_navigatable(self):
        ...
        visited = np.zeros(self.floor.shape, dtype=bool)

        total = visited.shape[0] * visited.shape[1] - len(self.furniture_tiles)

        visited[start.get_y(True), start.get_x(True)] = True
        ...
        while queue:
            pos = queue.pop(0)
            for d in directions:
                new_pos = Position(pos.get_x(True) + d[0], pos.get_y(True) + d[1])
                if self.is_position_valid(new_pos) and not visited[new_pos.get_y(True), new_pos.get_x(True)]:
                    visited[new_pos.get_y(True), new_pos.get_x(True)] = True
                    queue.append(new_pos)

        return np.sum(visited) == total
    ...
    def add_furniture_to_room(self):
        ...
        Furniture location and size are randomly selected. Width and height are selected
        ...
        for _ in range(max_attempts):
            # Generate random furniture dimensions and position
            furniture_width = random.randint(1, self.floor.shape[1] - 1)
            furniture_height = random.randint(1, self.floor.shape[0] - 1)

            f_bottom_left_x = random.randint(0, self.floor.shape[1] - furniture_width)
            f_bottom_left_y = random.randint(0, self.floor.shape[0] - furniture_height)

            # Create a list of tiles to be furnished
            proposed_furniture = [
                (i, j)
            ...
            # Temporarily add the proposed furniture to the furniture_tiles list
            self.furniture_tiles.extend(proposed_furniture)

            # Check if the room is still navigable
            if self.is_navigatable():
                # The placement is valid, break out of the loop
                return
            else:
                # Placement invalid, remove the proposed furniture and try again
                self.furniture_tiles = [
                    tile for tile in self.furniture_tiles if tile not in proposed_furniture
                ]

    # If no valid furniture placement is found after max_attempts
    raise ValueError("Unable to add furniture without blocking paths.")
    ...
    def is_position_furnished(self, pos):
        ...
        """
        return self.is_tile_furnished(pos.get_x(True), pos.get_y(True))
    ...
    def get_num_tiles(self):
        ...
        """
        return self.floor.shape[0] * self.floor.shape[1] - len(self.furniture_tiles)
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        positions = [(x, y) for x in range(self.floor.shape[1]) for y in range(self.floor.shape[0])]
        available = list(set(positions) - set(self.furniture_tiles))
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_position = position.get_new_position(direction, self.speed)

        if self.room.is_position_valid(new_position):
            self.set_robot_position(new_position)
            self.room.clean_tile_at_position(new_position, self.clean_capacity)
        else:
