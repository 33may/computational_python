...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if self.has_node(node):
            raise ValueError('Node already exists in Digraph')
        else:
            self.nodes.add(node)
    ...
    def add_edge(self, edge):
        ...
        in the  graph."""
        if self.has_node(edge.src) and self.has_node(edge.dest):
            self.edges.setdefault(edge.src, []).append(edge)
        else:
            raise ValueError('Edge src and dest must be in the Digraph')
